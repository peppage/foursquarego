...
func TestVenueService_Details(t *testing.T) {
	...
	assert.Equal(t, 767, venue.Likes.Groups[0].Count)
	...
	assert.Equal(t, 1, venue.HereNow.Groups[0].Count)
	...
	assert.Equal(t, "http://www.newyorker.com/magazine/2015/06/29/bar-tab-threes-brewing", venue.Tips.Groups[0].Items[0].URL)
	assert.Equal(t, "https://foursquare.com/item/523e7cb811d2c579d576a7d7", venue.Tips.Groups[0].Items[0].CanonicalURL)
	assert.Len(t, venue.Tips.Groups[0].Items[0].Likes.Groups, 1)
	assert.Len(t, venue.Tips.Groups[0].Items[0].Likes.Groups[0].Items, 1)
	assert.Equal(t, false, venue.Tips.Groups[0].Items[0].Like)
	assert.Equal(t, true, venue.Tips.Groups[0].Items[0].LogView)
	...
	assert.Equal(t, 0, venue.Tips.Groups[0].Items[0].Todo.Count)
	...
	assert.Equal(t, 1444940304, venue.Listed.Groups[0].Items[0].ListItems.Items[0].CreatedAt)
	...
	assert.Equal(t, "... of guest taps. Rotating kitchen of good Brooklyn faire.", venue.Phrases[0].Sample.Text)
